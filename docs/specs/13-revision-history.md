...
| 1.2 | 2026-07-10 | chủ dự án | Quyết định chủ dự án (đã ký) vá lỗ hổng độ trung thực OCR: sửa AC-02.6 và OI-07 để thêm lớp khai báo độ trung thực theo ngôn ngữ của trait `OcrEngine` (`Full` vs `Degraded{reason}`) và một notice cố định cho trạng thái `Degraded` dù confidence cao. Bối cảnh: PP-OCRv5/oar-ocr với tiếng Việt bị charset-cap (~0.74), rec dict thiếu diacritic tổ hợp U+1E00-U+1EFF nên dấu thanh bị rơi trong khi confidence vẫn ~0.97 và không bị ngưỡng `PerLine` bắt (giữ đúng BR-05). Đồng bộ PRD-FR-02 và ghi known-issue. Không sửa ADR-004 (Accepted, bất biến). |
