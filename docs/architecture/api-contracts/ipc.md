...
### `region:translation-delta` -> `TranslationDeltaPayload`

Phát sau MỖI chunk văn bản không rỗng nhận được từ `TranslationProvider::translate_stream`
(`region.rs::run_translation_stream`), trước sự kiện chung cuộc `region:translation-result`
(sửa lỗi timeout giả - xem bên dưới).

| Trường      | Kiểu     | Ghi chú                                                                              |
| ----------- | -------- | ------------------------------------------------------------------------------------- |
| `requestId` | `string` | Phải khớp yêu cầu đang chờ; delta lạc (từ yêu cầu đã bị thay thế) bị bỏ qua.          |
| `text`      | `string` | Văn bản dịch TÍCH LŨY tới thời điểm này (không phải delta thô của riêng chunk) - UI render trực tiếp, không tự nối chuỗi. Coi là DATA không tin cậy (render qua `PlainText`, giống `region:translation-result`). |

### `region:translation-result` -> `TranslationResultPayload`

Phát khi stream dịch kết thúc thành công (chung cuộc, sau 0+ sự kiện `region:translation-delta`
ở trên).

| Trường           | Kiểu     | Ghi chú                                                    |
| ---------------- | -------- | --------------------------------------------------------- |
| `requestId`      | `string` | Phải khớp yêu cầu đang chờ; phản hồi lạc bị bỏ qua.       |
| `translatedText` | `string` | Bản dịch đầy đủ (render plain-text).                       |
...
Nếu không có sự kiện nào tới (không delta, không result, không error), UI tự chuyển sang
trạng thái thất bại sau ngưỡng timeout (`TRANSLATION_TIMEOUT_MS`, hiện 8000 ms) - dư dả so với
NFR-PERF-02 (region p95 < 2s) nhưng vẫn bảo đảm không treo vô hạn. FIX lỗi timeout giả (owner
report): delta ĐẦU TIÊN của một yêu cầu xoá bộ đếm timeout ngay - một stream đang thực sự trả
văn bản (dù chậm) không phải là "treo", nên không được phép hiện lỗi đỏ trước khi kết quả thật
tới. Chỉ một stream không phát ra GÌ trong ngân sách mới là timeout thật.
...
sequenceDiagram
    ...
    loop Mỗi chunk stream không rỗng
        Core-->>UI: region:translation-delta (text tích lũy)
        Note over UI: delta ĐẦU TIÊN xoá timeout ngay (stream đang sống)
    end
    alt Dịch thành công
        Core-->>UI: region:translation-result (translatedText, provider, model)
    else Thất bại / huỷ
        Core-->>UI: region:translation-error (message?)
    else Không delta, không result, không error
