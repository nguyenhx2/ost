...
## Result
Landed via PR #57, merge commit `f7a31a3`. Implemented `src-tauri/src/shell/windows.rs::open_deferred`, which never calls `WebviewWindowBuilder::build()` on the calling turn: it spawns an OS thread that calls `AppHandle::run_on_main_thread`, forcing the queued event-loop-proxy path (a same-thread call would run the build inline and reproduce the deadlock). All five window-creation sites routed through it (caption, history, settings, region select, region preview). Anti-regression guard: `cargo test` test `no_raw_webview_window_builder_outside_this_module` recursively scans `src/shell/**/*.rs` and fails if a raw `WebviewWindowBuilder` reappears outside `windows.rs`. Proven to go red on a reintroduced inline build (including in a nested submodule). New e2e spec `e2e/specs/window-open-deferred.spec.ts` drives `open_caption_overlay`, `open_settings`, `open_history` from a real WebView IPC callback and asserts both liveness AND that the windows are actually created. Empirical verification on the RELEASE binary (orchestrator-run): UNFIXED build = 0/4 e2e tests pass, every WebView script call times out, main thread parked in a nested `GetMessageA` pump beneath `EmbeddedBrowserWebView!...FireWebResourceRequestedEvent` (the reentrant deadlock). FIXED build = 4/4 pass, liveness probes 22-24ms, windows actually built, main thread a clean top-level `GetMessageW` loop with zero WebView2-callback frames. Code review blocker fixed: `show()`/`set_focus()` errors now logged via `tracing::error!`.
